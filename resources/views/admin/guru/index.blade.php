...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 page-header">
        <div>
            <h1 class="h3">Kelola Guru</h1>
            <p>Daftar tenaga pendidik yang ditampilkan di website sekolah.</p>
        </div>
                ...
                @forelse ($gurus as $guru)
                    <tr>
                        <td>
                            @if ($guru->foto)
                                @php
                                    $namaFoto = basename(str_replace('\\', '/', $guru->foto));
                                @endphp

                                @if (file_exists(public_path('images/guru/' . $namaFoto)))
                                    <img
                                        src="{{ asset('images/guru/' . $namaFoto) }}"
                                        alt="{{ $guru->nama }}"
                                        class="foto-thumb"
                                    >
                                @else
                        ...
                        <td class="nama-guru">{{ $guru->nama }}</td>

                        <td class="nip-text">{{ $guru->nip ?? '-' }}</td>

                        <td class="mapel-text">{{ $guru->mapel ?? '-' }}</td>
